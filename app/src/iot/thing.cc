...
Thing* CreateThing(const std::string& type) {
    ...
    if (creator == thing_creators->end()) {
        rt_kprintf(TAG, "Thing type not found: %s", type.c_str());
        return nullptr;
    }
...
void Thing::Invoke(const cJSON* command) {
    ...
    for (auto& param : method.parameters()) {
        ...
        if (param.required() && input_param == nullptr) {
            ...
            rt_kprintf(TAG, "Parameter %s is required", param.name().c_str());
        }
    ...
    //     rt_kprintf(TAG, "Method not found: %s", method_name->valuestring);
    //     return;
    // }
}
